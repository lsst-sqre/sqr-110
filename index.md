...
After that, top-level routes (e.g. `/tutorials` or `/queries`) were given `GafaelfawrIngresses` to redirect those routes to Ghostwriter rewrite routes, which in turn triggered Ghostwriter hooks to substitute parameters as necessary and ultimately return a redirect to a Lab with a templated query or a tutorial opened in a notebook.

The proposed design reworks the `hook` functionality as a set of pluggable classes, which can be loaded at startup time, and the service that contains these classes as conceptually distinct from template rendering and external system management.
...
Whether anyone will take advantage of this flexibility is as yet uknown.

Although SQuaRE will supply implementations for our initial set of parser classes needed to implement templated queries and tutorial access, any Python class that meets the restrictions of the parser interface (loading a list of chained parsers and a route-to-head-of-parser-chain map at startup, combined with correctly-typed implementations of `handle_request()` and `rewrite()`) can be used as a parser class.

Note that in order to deploy a new parser, the developer will have to work closely with a [Phalanx](https://phalanx.lsst.io) administrator, because each use of `handle_request()` will be coupled to a particular `GafaelfwarIngress` defined in a Phalanx application template.
Therefore a developer of parser functionality will have to also write Helm YAML for the external routes' `GafaelfawrIngresses` and configure the list of parser classes for each route in order to have the parser take effect.
...
The external route is optional for a given parser class; however, for a given route, the first class in any parser chain must be bound to an external route.
Subsequent classes will only be invoked via the chain rather than by an external request.

The parser class also exposes two methods, `handle_request()` and `rewrite()`.
...
`Dixon` will vary from our usual FastAPI application pattern.
...
1. `render`: Request a document and return that document directly to the caller.
   This document may have parameters subsituted into a template via user input.
2. `write-file`: Perform the actions of `render`, and then write that document into POSIX file space accessible to the user.
3. `open-notebook`: Perform the actions of both `render` and `write-file`, and then open a JupyterLab instance open to the document within the user's file space.
...
Once the JupyterLab-file-browser-relative path is determined, the path-in-storage can be translated to a path underneath a `/tree` URL in the running Lab.
...
If a document is being returned, the content-type should be `application/json` for a `Notebook` type query.
...
If the mode is `open-notebook`, a successful request will return a JSON object containing a `url` field indicating the running user Lab open to the correct document.
...
User labs also contain a backend extension implementing a proxy for the Ghostwriter service.
This extension itself will need to remain.
In order to ensure a running lab and then redirect the user to a specific path within that lab, the Ghostwriter extension must be specified as the target page and given appropriate inputs.
What it returns will guide the user's browser to the correct destination.
This is the technique called the "bank shot" in the current implementation.

This same technique can also be used, once we have a user preference mechanism, to determine whether to redirect the user browser to the tutorial landing page or the default Lab UI.
